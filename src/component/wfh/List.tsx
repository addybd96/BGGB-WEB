...
import { approveWFH } from '../../action/WFHActions'
...
export default class CompOffList extends React.Component<any, any>
{
    ...
    empCodeCellFormatter = (cell: any, row: any, rowIndex: any, formatExtraData: any) => {
        return <Link to={`/wfh-list/${cell}`}>{cell}</Link>
    }
    ...
    renderLeavesTable = () => {
        return this.props.wfh.map((leave: any, lIndex: number) => {
            return (<tr key={lIndex}>
                <td className="text-center"><Link to={`/wfh-list/${leave.employeeId}`}>{leave.employeeId}</Link></td>
    ...
    onSubmit = (payload: any) => {
        ...
        console.log(payload)
        approveWFH(payload).then((resp: any) => {
            this.setState({ isModalOpen: false })
