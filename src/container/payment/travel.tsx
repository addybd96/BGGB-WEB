...
import { getTrevelAlneByUserId } from "../../action/payment";
...
class ComponentName extends React.Component<any, any> {
              ...
              <div className="card mt-3">
                <div className="card-header">
                  ...
                  <div className="pull-right ">
                    ...
                    {
                        this.state.selectedPending && <button
                        type="button"
                          ...
                          list.map((l: any, i: any) => {
                            return (
                              <tr key={i}>
                                ...
                                <td className="text-center">{dateFormat(l.date, 'mmmm-yyyy')}</td>
                                <td className="text-center">
                                  {" "}
                                  {l.trndate
                                ...
                                {/* <td className="text-center">
                                  <Link
                                    to={`${CONSTANT.url.payment.updateTrans}?id=${l.id}&name=${l.name}(${l.employeeId})&sch=perquisites&tb=travelAllowance`}
                                  >
                                    <i
  ...
  downloadToExcel = () => {
    ...
    let filename = "Vechicle-allowance.xlsx";
    ...
    const data = list
      ...
      .map((item: any, index: number) => {
        ...
        dataToExport.push({
          ...
          Sol :item.soul,
          Particular: `Fuel allowance for the month of ${dateFormat(
            item.date,
            "mmmm-yyyy"
          )}`,
    ...
    const data = list
      ...
      .map((item: any, index: number) => {
        ...
        dataToExport.push({
          ...
          "Name": item.name,
          "Amount": item.actualAmount,
          "Particular":`Full allowance for the month of ${dateFormat(item.date, 'mmmm-yyyy')}`,
          "Account Number": item.accountNumber,
        });
      });
    var wb = XLSX.utils.book_new(),
