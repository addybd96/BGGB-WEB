...
import { getUnifromAlneByUserId } from "../../action/payment";
...
class ComponentName extends React.Component<any, any> {
                          ...
                          list.map((l: any, i: any) => {
                            return (
                              <tr key={i}>
                                ...
                                {/* <td><button className=" btn btn-secondary" name = {l.userId}  value={l.name}  onClick={(e)=>this.downloadExcel(e)}>Download Excel</button> </td> */}
                                ...
                                {/* <td className="text-center">
                                                                    <Link to={`${CONSTANT.url.payment.updateTrans}?id=${l.id}&name=${l.name}(${l.employeeId})&sch=perquisites&tb=uniformAndLiveries`} >
    ...
    const data = list
      ...
      .map((item: any, index: number) => {
        ...
        dataToExport.push({
          ...
          Sol:item.soul,
          Particular: `${item.allowanceType} for the block of (${dateFormat(
            oldDate,
            "mmmm-yyyy"
          )} - ${dateFormat(
            item.toDate,
  ...
  loadData = (status?: any) => {
    ...
    getUnifromAlneByUserId({ status }).then((res: any) => {
      status
        ? this.setState({
            showLoader: false,
            list: res ? res : "",
      ...
      for (let i = 0; i < res.length; i++) {
        let tempRes = {
          ...
          allowanceType: res[i] ? res[i].allowanceType : "",
