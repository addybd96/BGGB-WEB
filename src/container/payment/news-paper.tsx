...
import { getNewsPaperAlneByUserId } from "../../action/payment";
...
class ComponentName extends React.Component<any, any> {
                          ...
                          list.map((l: any, i: any) => {
                            return (
                              <tr key={i}>
                                ...
                                {/* <td className="text-center">
                                                                    <Link to={`${CONSTANT.url.payment.updateTrans}?id=${l.id}&name=${l.name}(${l.employeeId})&sch=perquisites&tb=newsPaperAllowance`} >
  ...
  downloadToExcel = () => {
    ...
    let filename = "news-paper-allowance.xlsx";
    ...
    const data = list
      ...
      .map((item: any, index: number) => {
        const oldDate= new Date(dateFormat(item.date, "mmmm-yyyy"));
        var fiscalyear = "";
        if ((oldDate.getMonth() + 1) <= 3) {
          fiscalyear = (oldDate.getFullYear() - 1) + "-" + oldDate.getFullYear()
        } else {
          fiscalyear = oldDate.getFullYear() + "-" + (oldDate.getFullYear() + 1)
        }
        delete item.code;
        dataToExport.push({
          ...
          Sol:item.soul,
          Particular: `News paper allowance for the F.Y. ${fiscalyear}`,
