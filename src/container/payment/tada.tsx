...
import { getTadaAlneByUserId } from "../../action/payment";
...
class ComponentName extends React.Component<any, any> {
                    ...
                    <table className="table table-striped">
                      <thead>
                        <tr>
                          ...
                          <th className="text-center"> Fare Paid</th>
                          {/* <th className="text-center"> Download Excel</th> */}
                          ...
                          list.map((l: any, i: any) => {
                            return (
                              <tr key={i}>
                                ...
                                {/* <td className="text-center">
                                  <Link
                                    to={`${CONSTANT.url.payment.updateTrans}?id=${l.id}&name=${l.name}(${l.employeeId})&sch=perquisites&tb=tadaAllowance`}
                                  >
                                    <i
  ...
  downloadToExcel = () => {
    ...
    let filename = "Tada-allowance.xlsx";
    ...
    const data = list
      ...
      .map((item: any, index: number) => {
        ...
        dataToExport.push({
          ...
          Sol:item.soul,
          Particular: `Full allowance for the month of ${dateFormat(
            item.date,
            "mmmm-yyyy"
          )}`,
  ...
  loadData = (status?: any) => {
    ...
    getTadaAlneByUserId({ status }).then((res: any) => {
      ...
      for (let i = 0; i < res.length; i++) {
        let tempRes = {
          departurePalace: res[i] ? res[i].departurePalace : "",
          departureDate: res[i] ? res[i].departureDate : "",
          arrivalPalace: res[i] ? res[i].arrivalPalace : "",
          arrivalDate: res[i] ? res[i].arrivalDate : "",
          farePaid: res[i] ? res[i].farePaid : "",
          autoRixaExpense: res[i] ? res[i].autoRixaExpense : "",
          hotelAmountPaidPerDay: res[i] ? res[i].hotelAmountPaidPerDay : "",
          purpose: res[i] ? res[i].purpose : "",
          totalTada: res[i] ? res[i].totalTada : "",
          accountNumber: res[i] ? res[i].accountNumber : "",
          actualAmount: res[i] ? res[i].totalAmount : "",
