...
import { getMedicalAlneByUserId } from "../../action/payment";
...
class ComponentName extends React.Component<any, any> {
                          ...
                          list.map((l: any, i: any) => {
                            return (
                              <tr key={i}>
                                ...
                                {/* <td className="text-center">
                                  <Link
                                    to={`${CONSTANT.url.payment.updateTrans}?id=${l.id}&name=${l.name}(${l.employeeId})&sch=perquisites&tb=medicalAllowance`}
                                  >
                                    <i
  ...
  downloadToExcel = () => {
    ...
    let filename = "Medical-allowance.xlsx";
    ...
    const data = list
      ...
      .map((item: any, index: number) => {
        ...
        dataToExport.push({
          ...
          Sol:item.soul,
          Particular: `Medical allowance for the year ${dateFormat(
            item.fromDate,
            "mmmm-yyyy"
          )}`,
  ...
  loadData = (status?: any) => {
    ...
    getMedicalAlneByUserId({ status }).then((res: any) => {
      ...
      for (let i = 0; i < res.length; i++) {
        let tempRes = {
          fromDate: res[i] ? res[i].fromDate : "",
          toDate: res[i] ? res[i].toDate : "",
          illnessNature: res[i] ? res[i].illnessNature : "",
          treatmentDuration: res[i] ? res[i].treatmentDuration : "",
          physicianFee: res[i] ? res[i].physicianFee : "",
          diagnosticCharges: res[i] ? res[i].diagnosticCharges : "",
          medicalMedicine: res[i] ? res[i].medicalMedicine : "",
          injection: res[i] ? res[i].injection : "",
          xRayInvestigation: res[i] ? res[i].xRayInvestigation : "",
          clinicMedicine: res[i] ? res[i].clinicMedicine : "",
          others: res[i] ? res[i].others : "",
          total: res[i] ? res[i].total : "",
