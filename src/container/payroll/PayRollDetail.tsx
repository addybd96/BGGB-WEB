...
import { getPayRollList, getPayRollListCount, revertPayRoll } from '../../action/PayrollCycleAction';
...
class ComponentName extends React.Component<any, any> {
    ...
    public render() {
        const { list, page, limit, sort, count, date, showLoader, ttumData } = this.state;
        ...
        const columns = [
            ...
            {
                dataField: 'cycleAmount',
                text: 'Cycle Allowance',
                style: {
                    fontSize: '14px'
                }
            ...
            {
                dataField: 'societyAmount',
                text: 'Welfare Association',
                style: {
                    fontSize: '14px'
                }
                            ...
                            <div className="row mt-4">
                                <div className="col-12 col-lg-4">
                                    <div className="card attendance text-left">
                                        <div className="card-header">
                                            <b>{date.value ? moment(date.value).format('MMMM YYYY') : moment().format('MMMM YYYY')} | {date.value ? moment(date.value).daysInMonth() : moment().daysInMonth()} Payable Days</b>
                                        </div>
                                            ...
                                            <div className="row">
                                                <div className="col-lg-12 hours-left">
                                                    <p><strong>
                                                        {
                                                            ttumData
                                                                ? new Intl.NumberFormat('en-IN', { maximumSignificantDigits: 7 }).format(ttumData.reduce((pre: number, { totalAllow }: any) => (Number(totalAllow) + pre), 0))
                                                                : 0
                                                        }
                                                    </strong></p>
                                                ...
                                                <div className="col-lg-12 hours-left">
                                                    <p><strong>
                                                        {
                                                            ttumData
                                                                ? new Intl.NumberFormat('en-IN', { maximumSignificantDigits: 7 }).format(ttumData.reduce((pre: number, { net }: any) => (Number(net) + pre), 0))
                                                                : 0
                                                        }
                                                    </strong></p>
                                ...
                                <div className="col-12 col-lg-4">
                                    <div className="card attendance text-left">
                                        ...
                                        <div className="text-center  mt-4"><h3>{date.value ? moment(date.value).daysInMonth() : moment().daysInMonth()}</h3></div>
                                                ...
                                                <div className="row">
                                                    <div className="col-12 col-lg-12 leave-date">
                                                        ...
                                                        <span>
                                                            {
                                                                ttumData
                                                                    ? Math.round(ttumData.reduce((pre: number, { inctAmount }: any) => (Number(inctAmount) + pre), 0))
                                                                    : 0
                                                            }
                                                        </span>
                                                    </div>
                                                ...
                                                <div className="row">
                                                    <div className="col-12 col-lg-12 leave-date">
                                                        ...
                                                        <span>
                                                            {
                                                                ttumData
                                                                    ? Math.round(ttumData.reduce((pre: number, { prov }: any) => (Number(prov) + pre), 0))
                                                                    : 0
                                                            }
                                                    ...
                                                    <div className="col-12 col-lg-12 leave-date">
                                                        <p className="mb-0">NPS Deductions</p>
                                                        <span>
                                                            {
                                                                ttumData
                                                                    ? Math.round(ttumData.reduce((pre: number, { nps }: any) => (Number(nps) + pre), 0))
                                                                    : 0
                                                            }
                                                    ...
                                                    <div className="col-12 col-lg-12 leave-date">
                                                        ...
                                                        <span>
                                                            {
                                                                ttumData
                                                                    ? Math.round(ttumData.reduce((pre: number, { profTax }: any) => (Number(profTax) + pre), 0))
                                                                    : 0
                                                            }
                                                    ...
                                                    <div className="col-12 col-lg-12 leave-date">
                                                        <p className="mb-0">GSLI Deductions</p>
                                                        <span>
                                                            {
                                                                ttumData
                                                                    ? Math.round(ttumData.reduce((pre: number, { gpAmount }: any) => (Number(gpAmount) + pre), 0))
                                                                    : 0
                                                            }
                            ...
                            <div className="card mt-3">
                                <div className="card-header">
                                    ...
                                    <div className="float-right col-lg-2">
                                        <b><Link to={CONSTANT.url.payrollCycleList}>Process PayRoll</Link> </b>
    ...
    getSalarySlip = (cell: any, row: any, rowIndex: any, formatExtraData: any) => {
        const detailPageURL = `${CONSTANT.url.salarySlip}?id=${row.id}&date=${row.date}`;
    ...
    onTableChange = (type: any, { page, sizePerPage }: any) => {
        ...
        if (!isNaN(page))
            getPayRollList(page, sizePerPage, 1, this.state.date.value || new Date()).then((response: any) => {
                window.scrollTo(0, 0)
                this.setState({
    ...
    submit = (status: any) => {
        ...
        revertPayRoll({ status, date: this.state.date.value || new Date() }).then((res: any) => {
    ...
    loadList = () => {
        ...
        getPayRollList(page, limit, sort, d).then((res: any) => {
            if (res && res.result)
    ...
    count = () => {
        ...
        const d = date.value || moment().format('YYYY-MM-DD')
        getPayRollListCount(d).then((res: any) => {
            if (res && res.result) {
    ...
    downloadXL = () => {
        ...
        let dataToExport = this.state.ttumData.map((item: any, index: any) => {
            ...
            return {
                ...
                "Net salary": item.net,
                "Working Days": item.workingDay,
                "Totla Days": moment().daysInMonth(),
